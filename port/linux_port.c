...
static void set_dtr_and_rts(int dtr, int rts)
{
    int status = 0;
...
void loader_port_enter_bootloader(void)
{
    ...
    set_dtr_and_rts(1, 0);
    usleep(100000);
    set_dtr_and_rts(0, 1); // IO0=LOW & EN=HIGH, chip out of reset
    usleep(100000);
    set_dtr_and_rts(0, 0); // IO=HIGH, done

    usleep(100000);

    printf("RESET SEQUENCE DONE\n");
...
void loader_port_reset_target(void)
{
    set_dtr_and_rts(0, 0);
    set_dtr_and_rts(1, 1);
    set_dtr_and_rts(0, 1); // IO0=HIGH & EN=LOW, chip in reset
}
